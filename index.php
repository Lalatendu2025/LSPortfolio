<!DOCTYPE html>
<html lang="en">
<!--Doctype Start Here-->
<?php include( "includes/doctype.php" );?>
<!--Doctype Ends Here-->


<body class="customScrolddl">
   <main class="wrapper">
      <!--Header Start Here-->
      <?php include( "includes/header.php" ); ?>
      <!--Header Ends Here-->

      <!--Banner Start Here-->
      <?php include( "includes/banner.php" ); ?>
      <!--Banner Ends Here-->
      <section class="linkRoller">
         <!-- Slider main container -->
         <div class="swiper">
            <!-- Additional required wrapper -->
            <div class="swiper-wrapper">
               <!-- Slides -->
               <div class="swiper-slide"><i class="las la-snowflake"></i> Graphics Design</div>
               <div class="swiper-slide"><i class="las la-snowflake"></i> Website Design</div>
               <div class="swiper-slide"><i class="las la-snowflake"></i> App Design</div>
               <div class="swiper-slide"><i class="las la-snowflake"></i> Product Design</div>
               <div class="swiper-slide"><i class="las la-snowflake"></i> UI/UX Design</div>
               <div class="swiper-slide"><i class="las la-snowflake"></i> Responsive Design</div>
            </div>

         </div>
      </section>

      <section class="myServices">
         <div class="container">
            <div class="row align-items-center justify-content-between">
               <div class="col">
                  <h2 class="sectionTitle">
                     <small class="subTitle"><strong>- </strong>My Services</small>
                     <span class="mainTitle"><em>Services</em> I Provide</span>
                  </h2>
               </div>
               <div class="col text-end"><button class="btn-lg customBtn"><span>View All Services</span><i
                        class="las la-long-arrow-alt-right"></i></button></div>
            </div>
            <div class="row mt-5">
               <div class="col-sm-3">
                  <div class="myServiceBox">
                     <span><i class="las la-palette"></i></span>
                     <h3>UI/UX Design</h3>
                     <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Vero, facere labore! Minima ducimus
                        iusto, magnam enim ea natus dolor.</p>
                     <a href="#">Learn More <i class="las la-long-arrow-alt-right"></i></a>
                  </div>
               </div>
               <div class="col-sm-3">
                  <div class="myServiceBox">
                     <span><i class="las la-bezier-curve"></i></span>
                     <h3>App Design</h3>
                     <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Vero, facere labore! Minima ducimus
                        iusto, magnam enim ea natus dolor.</p>
                     <a href="#">Learn More <i class="las la-long-arrow-alt-right"></i></a>
                  </div>
               </div>
               <div class="col-sm-3">
                  <div class="myServiceBox">
                     <span><i class="las la-globe"></i></span>
                     <h3>Website Design</h3>
                     <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Vero, facere labore! Minima ducimus
                        iusto, magnam enim ea natus dolor.</p>
                     <a href="#">Learn More <i class="las la-long-arrow-alt-right"></i></a>
                  </div>
               </div>
               <div class="col-sm-3">
                  <div class="myServiceBox">
                     <span><i class="las la-boxes"></i></span>
                     <h3>Product Design</h3>
                     <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Vero, facere labore! Minima ducimus
                        iusto, magnam enim ea natus dolor.</p>
                     <a href="#">Learn More <i class="las la-long-arrow-alt-right"></i></a>
                  </div>
               </div>
            </div>
         </div>
      </section>

      <section class="aboutMe">
         <div class="container">
            <div class="row align-items-center">
               <div class="col-sm-6">
                  <figure>
                     <img src="assets\img\frameImage.png" alt="Frame Image" class="img-fluid">
                  </figure>
               </div>
               <div class="col-sm-6">
                  <h2 class="sectionTitle">
                     <small class="subTitle"><strong>- </strong>About Me</small>
                     <span class="mainTitle">Who is <em>Lalatendu Sarangi?</em></span>
                  </h2>
                  <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Commodi magnam unde saepe architecto, quod
                     corrupti quaerat est.</p>
                  <div class="row mb-5">
                     <div class="col-sm-4">
                        <h5>100+<small> Projects Completed</small></h5>
                     </div>
                     <div class="col-sm-4">
                        <h5>50+<small> Happy Clients</small></h5>
                     </div>
                     <div class="col-sm-4">
                        <h5>5+<small> Years of Experience</small></h5>
                     </div>
                  </div>
                  <div class="row align-items-center justify-content-start">
                     <div class="col">
                        <button class="btn-lg customBtn"><span>Download CV</span><i
                              class="las la-long-arrow-alt-right"></i></button>

                     </div>
                     <div class="col">
                        <ul class="mySocialMedia">
                           <li><a href="#"><i class="lab la-facebook-f"></i></a></li>
                           <li><a href="#"><i class="lab la-instagram"></i></a></li>
                           <li><a href="#"><i class="lab la-twitter"></i></a></li>
                           <li><a href="#"><i class="lab la-linkedin-in"></i></a></li>
                           <li><a href="#"><i class="lab la-youtube"></i></a></li>
                        </ul>
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </section>

      <section class="myFabTools">
         <div class="container-fluid">
            <div class="row mb-5">
               <div class="col text-center">
                  <h2 class="sectionTitle">
                     <small class="subTitle"><strong>- </strong>My Services</small>
                     <span class="mainTitle"><em>Exploring the Tools</em> Behind My Design</span>
                  </h2>
               </div>
            </div>
            <div class="row">
               <div class="col p-0">
                  <!-- Slider main container -->
                  <div class="swiper">
                     <!-- Additional required wrapper -->
                     <div class="swiper-wrapper">
                        <!-- Slides -->
                        <div class="swiper-slide">
                           <span class="myFabToolBox">
                              <figure>
                                 <img src="assets\img\toolicons\figma.png" alt="">
                              </figure>
                              <strong>98%</strong>
                              <h5>Figma</h5>
                           </span>
                        </div>
                        <div class="swiper-slide">
                           <span class="myFabToolBox">
                              <figure>
                                 <img src="assets\img\toolicons\photoshop.png" alt="">
                              </figure>
                              <strong>95%</strong>
                              <h5>Photoshop</h5>
                           </span>
                        </div>
                        <div class="swiper-slide">
                           <span class="myFabToolBox">
                              <figure>
                                 <img src="assets\img\toolicons\illustrator.png" alt="">
                              </figure>
                              <strong>90%</strong>
                              <h5>Illustrator</h5>
                           </span>
                        </div>
                        <div class="swiper-slide">
                           <span class="myFabToolBox">
                              <figure>
                                 <img src="assets\img\toolicons\framer-icon.webp" alt="">
                              </figure>
                              <strong>92%</strong>
                              <h5>Framer</h5>
                           </span>
                        </div>
                        <div class="swiper-slide">
                           <span class="myFabToolBox">
                              <figure>
                                 <img src="assets\img\toolicons\sketch-icon.webp" alt="">
                              </figure>
                              <strong>95%</strong>
                              <h5>Sketch</h5>
                           </span>
                        </div>
                        <div class="swiper-slide">
                           <span class="myFabToolBox">
                              <figure>
                                 <img src="assets\img\toolicons\3ds-max-icon.png" alt="">
                              </figure>
                              <strong>98%</strong>
                              <h5>3ds Max</h5>
                           </span>
                        </div>
                     </div>

                  </div>
               </div>
            </div>
      </section>


      <section class="myPortFolio">
         <div class="container">
            <div class="row mb-5 align-items-center justify-content-between">
               <div class="col">
                  <h2 class="sectionTitle">
                     <small class="subTitle"><strong>- </strong>My Portfolio</small>
                     <span class="mainTitle">My Latest <em>Projects</em></span>
                  </h2>
               </div>
               <div class="col text-end"><button class="btn-lg customBtn"><span>View All Projects</span><i
                        class="las la-long-arrow-alt-right"></i></button></div>
            </div>
            <div class="row g-5">
               <div class="col-sm-4">
                  <div class="myProjectBox">
                     <figure class="projectImage">
                        <img src="assets/img/portfolioImgs/CNFINDIA.png" alt="">
                     </figure>
                     <ul class="projectTags">
                        <li>Photoshop</li>
                        <li>Figma</li>
                        <li>UI/UX</li>
                     </ul>
                     <div class="projectBoxFooter">
                        <h5>CNF India</h5>
                        <a href="#" class="btn btn-main"><i class="las la-long-arrow-alt-right"></i></a>
                     </div>
                  </div>
               </div>
               <div class="col-sm-4">
                  <div class="myProjectBox">
                     <figure class="projectImage">
                        <img src="assets/img/portfolioImgs/tea-talk.jpg" alt="">
                     </figure>
                     <ul class="projectTags">
                        <li>Web Design</li>
                        <li>UI/UX</li>
                     </ul>
                     <div class="projectBoxFooter">
                        <h5>Tea Talk</h5>
                        <a href="#" class="btn btn-main"><i class="las la-long-arrow-alt-right"></i></a>
                     </div>
                  </div>
               </div>
               <div class="col-sm-4">
                  <div class="myProjectBox">
                     <figure class="projectImage">
                        <img src="assets/img/portfolioImgs/Dairy-Farm.png" alt="">
                     </figure>
                     <ul class="projectTags">
                        <li>Web Design</li>
                        <li>UI/UX</li>
                     </ul>
                     <div class="projectBoxFooter">
                        <h5>Dairy Farm</h5>
                        <a href="#" class="btn btn-main"><i class="las la-long-arrow-alt-right"></i></a>
                     </div>
                  </div>
               </div>
               <div class="col-sm-4">
                  <div class="myProjectBox">
                     <figure class="projectImage">
                        <img src="assets/img/portfolioImgs/WebDew.png" alt="">
                     </figure>
                     <ul class="projectTags">
                        <li>Web Design</li>
                        <li>UI/UX</li>
                     </ul>
                     <div class="projectBoxFooter">
                        <h5>Web Dew</h5>
                        <a href="#" class="btn btn-main"><i class="las la-long-arrow-alt-right"></i></a>
                     </div>
                  </div>
               </div>
               <div class="col-sm-4">
                  <div class="myProjectBox">
                     <figure class="projectImage">
                        <img src="assets/img/portfolioImgs/CNFINDIA.png" alt="">
                     </figure>
                     <ul class="projectTags">
                        <li>Photoshop</li>
                        <li>Figma</li>
                        <li>UI/UX</li>
                     </ul>
                     <div class="projectBoxFooter">
                        <h5>CNF India</h5>
                        <a href="#" class="btn btn-main"><i class="las la-long-arrow-alt-right"></i></a>
                     </div>
                  </div>
               </div>
               <div class="col-sm-4">
                  <div class="myProjectBox">
                     <figure class="projectImage">
                        <img src="assets/img/portfolioImgs/tea-talk.jpg" alt="">
                     </figure>
                     <ul class="projectTags">
                        <li>Web Design</li>
                        <li>UI/UX</li>
                     </ul>
                     <div class="projectBoxFooter">
                        <h5>Tea Talk</h5>
                        <a href="#" class="btn btn-main"><i class="las la-long-arrow-alt-right"></i></a>
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </section>

      <section class="myCareer">
         <div class="container">
            <div class="row mb-5">
               <div class="col text-center">
                  <h2 class="sectionTitle">
                     <small class="subTitle"><strong>- </strong>Education & Work</small>
                     <span class="mainTitle">My<em> Academic and<br>Professional </em>Journey</span>
                  </h2>
               </div>
            </div>
            <div class="row g-5">
               <div class="col-sm-6">
                  <div class="myCareerBox">
                     <div class="mcHeader">
                        <span><i class="las la-graduation-cap"></i></span>
                        <h3>Education</h3>
                     </div>
                     <div class="mcBody">
                        <div class="swiper">
                           <div class="swiper-wrapper">
                              <div class="swiper-slide">
                                 <h6><small>2010-2013</small></h6>
                                 <h5>Utkal University</h5>
                                 <h6>Bachelor of Arts(Eng. Hons.)</h6>
                              </div>
                              <div class="swiper-slide">
                                 <h6><small>2008-2010</small></h6>
                                 <h5>Zed Career Academy</h5>
                                 <h6>Diploma in Multimedia(BBD)</h6>
                              </div>
                              <div class="swiper-slide">
                                 <h6><small>2004-2005</small></h6>
                                 <h5>Technosof Computer Education</h5>
                                 <h6>PGDCA</h6>
                              </div>
                              <div class="swiper-slide">
                                 <h6><small>2002-2004</small></h6>
                                 <h5>D.R. Nayapalli College</h5>
                                 <h6>+2 Science(PCMB)</h6>
                              </div>
                              <div class="swiper-slide">
                                 <h6><small>2002</small></h6>
                                 <h5>Unit6, Govt. High School, Bhubaneswar</h5>
                                 <h6>10th</h6>
                              </div>

                           </div>
                        </div>
                     </div>
                  </div>
               </div>
               <div class="col-sm-6">
                  <div class="myCareerBox">
                     <div class="mcHeader">
                        <span><i class="las la-briefcase"></i></span>
                        <h3>Work Experience</h3>
                     </div>
                     <div class="mcBody">
                        <div class="swiper">
                           <div class="swiper-wrapper">
                              <div class="swiper-slide">
                                 <h6><small>2024-2025</small></h6>
                                 <h5>Procentris India Pvt. Ltd.</h5>
                                 <h6>Sr. Web Designer</h6>
                              </div>
                              <div class="swiper-slide">
                                 <h6><small>2023-2024</small></h6>
                                 <h5>Swipewire Technologies</h5>
                                 <h6>Sr. Software Developer</h6>
                              </div>
                              <div class="swiper-slide">
                                 <h6><small>2021-2022</small></h6>
                                 <h5>Krazy-tech Business Solutions</h5>
                                 <h6>UI Developer</h6>
                              </div>
                              <div class="swiper-slide">
                                 <h6><small>2018-2021</small></h6>
                                 <h5>CSM Technologies Pvt. Ltd.</h5>
                                 <h6>Web Interface Developer</h6>
                              </div>
                              <div class="swiper-slide">
                                 <h6><small>2017-2018</small></h6>
                                 <h5>Future Focus Infotech</h5>
                                 <h6>Web Designer</h6>
                              </div>
                           </div>
                        </div>
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </section>

      <section class="myPricingList">
         <div class="container">
            <div class="row mb-5 align-items-center justify-content-between">
               <div class="col">
                  <h2 class="sectionTitle">
                     <small class="subTitle"><strong>- </strong>Pricing Table</small>
                     <span class="mainTitle">My <em>Pricing Model</em></span>
                  </h2>
               </div>
               <div class="col text-end"><button class="btn-lg customBtn"><span>Get Started</span><i
                        class="las la-long-arrow-alt-right"></i></button></div>
            </div>
            <div class="row g-5">
               <div class="col-sm-4">
                  <div class="pricingBox">
                     <div class="pbHeader">
                        <h2><strong>Hourly</strong><i class="las la-rupee-sign"></i>500 <small>/ Hour</small></h2>
                        <span><i class="lar la-money-bill-alt"></i></span>
                     </div>
                     <div class="pbBody">
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p class="disabled"><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                     </div>
                  </div>
               </div>
               <div class="col-sm-4">
                  <div class="pricingBox inverted">
                     <div class="pbHeader">
                        <h2><strong>Monthly</strong><i class="las la-rupee-sign"></i>1,00,000 <small>/ Month</small>
                        </h2>
                        <span><i class="lar la-money-bill-alt"></i></span>
                     </div>
                     <div class="pbBody">
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p class="disabled"><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                     </div>
                  </div>
               </div>
               <div class="col-sm-4">
                  <div class="pricingBox">
                     <div class="pbHeader">
                        <h2><strong>Quarterly</strong><i class="las la-rupee-sign"></i>3,50,000 <small>/ Quarter</small>
                        </h2>
                        <span><i class="lar la-money-bill-alt"></i></span>
                     </div>
                     <div class="pbBody">
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                        <p class="disabled"><i class="lar la-check-circle"></i> Copy icon & paste into design.</p>
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </section>


      <section class="myContactForm">
         <div class="container">
            <div class="row align-items-center justify-content-between">
               <div class="col-sm-5">
                  <div class="row">
                     <div class="col-12">
                        <h2 class="sectionTitle">
                           <small class="subTitle"><strong>- </strong>Contact Us</small>
                           <span class="mainTitle">Let's talk for<em> Your<br>Next Projects </em></span>
                        </h2>
                     </div>
                  </div>
                  <div class="row my-4">
                     <div class="col-12">
                        <p class="m-0">Lorem ipsum dolor sit amet consectetur adipisicing elit. Vitae laborum quo earum
                           quibusdam?</p>
                     </div>
                  </div>
                  <div class="row">
                     <div class="col-12">
                        <h5><span><i class="las la-phone"></i></span> +91-8917220478</h5>
                        <h5><span><i class="las la-envelope"></i></span> lalatendusarangi2025@gmail.com</h5>
                        <h5><span><i class="lab la-skype"></i></span> @Lalatendu2025</h5>
                        <h5><span><i class="las la-map-marker-alt"></i></span> M9/13, Panchasakhanagar, Jagamara,
                           Khandagiri, Bhubaneswar,<br> Odisha-751019</h5>
                     </div>
                  </div>
               </div>
               <div class="col-sm-6">
                  <form action="#" class="mcForm">
                     <div class="row mb-4">
                        <div class="col-6">
                           <div class="form-group">
                              <label for="name">Name</label>
                              <input type="text" id="name" class="form-control" placeholder="Your Name">
                           </div>
                        </div>
                        <div class="col-6">
                           <div class="form-group">
                              <label for="email">Email</label>
                              <input type="email" id="email" class="form-control" placeholder="Your Email">
                           </div>
                        </div>
                     </div>
                     <div class="row mb-4">
                        <div class="col-6">
                           <div class="form-group">
                              <label for="phone">Phone</label>
                              <input type="text" id="phone" class="form-control" placeholder="Your Phone Number">
                           </div>
                        </div>
                        <div class="col-6">
                           <div class="form-group">
                              <label for="ilike">I'm Interested in</label>
                              <select name="ilike" id="ilike" class="form-control">
                                 <option value="">Select an Option</option>
                                 <option value="web_design">Web Design</option>
                                 <option value="graphic_design">Graphic Design</option>
                                 <option value="seo">SEO</option>
                                 <option value="digital_marketing">Digital Marketing</option>
                              </select>
                           </div>
                        </div>
                     </div>
                     <div class="row mb-4">
                        <div class="col-6">
                           <div class="form-group">
                              <label for="budget">Budget Range</label>
                              <select name="budget" id="budget" class="form-control">
                                 <option value="">Select an Option</option>
                                 <option value="under_1000">Under $1,000</option>
                                 <option value="1000_5000">$1,000 - $5,000</option>
                                 <option value="5000_10000">$5,000 - $10,000</option>
                                 <option value="above_10000">Above $10,000</option>
                              </select>
                           </div>
                        </div>
                        <div class="col-6">
                           <div class="form-group">
                              <label for="country">Country</label>
                              <select name="country" id="country" class="form-control">
                                 <option value="">Select an Option</option>
                                 <option value="usa">USA</option>
                                 <option value="canada">Canada</option>
                                 <option value="uk">UK</option>
                                 <option value="australia">Australia</option>
                              </select>
                           </div>
                        </div>
                     </div>

                     <div class="row mb-4">
                        <div class="col-12">
                           <div class="form-group">
                              <label for="message">Message</label>
                              <textarea id="message" class="form-control" rows="5"
                                 placeholder="Your Message"></textarea>
                           </div>
                        </div>
                     </div>
                     <div class="row">
                        <div class="col-12">
                           <div class="form-group">
                              <button class="btn-lg customBtn"><span>Submit</span><i
                                    class="las la-long-arrow-alt-right"></i></button>
                           </div>
                        </div>
                     </div>

                  </form>
               </div>
            </div>
         </div>
      </section>


      <section class="clientReviews">
         <div class="container-fluid">
            <div class="row">
               <div class="col text-center">
                  <h2 class="sectionTitle">
                     <small class="subTitle"><strong>- </strong>Client Testimonials</small>
                     <span class="mainTitle">The Impact of My Work<br><em> Client Testimonials</em></span>
                  </h2>
               </div>
            </div>
            <div class="row mt-5 mb-4">
               <div class="col p-0">
                  <!-- Slider main container -->
                  <div class="swiper">
                     <!-- Additional required wrapper -->
                     <div class="swiper-wrapper">
                        <!-- Slides -->
                        <div class="swiper-slide">
                           <div class="clientReviewBox">
                              <h6 class="clientRating">
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <strong>5.0</strong>
                              </h6>
                              <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                              <div class="clientDetail">
                                 <figure>
                                    <img src="assets/img/clientIcon.png" alt="Client Image" class="img-fluid">
                                 </figure>
                                 <h5>John Doe<span>CEO, Example Company</span></h5>
                              </div>
                           </div>
                        </div>
                        <div class="swiper-slide">
                           <div class="clientReviewBox">
                              <h6 class="clientRating">
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <strong>5.0</strong>
                              </h6>
                              <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                              <div class="clientDetail">
                                 <figure>
                                    <img src="assets/img/clientIcon.png" alt="Client Image" class="img-fluid">
                                 </figure>
                                 <h5>John Doe<span>CEO, Example Company</span></h5>
                              </div>
                           </div>
                        </div>
                        <div class="swiper-slide">
                           <div class="clientReviewBox">
                              <h6 class="clientRating">
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <strong>5.0</strong>
                              </h6>
                              <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                              <div class="clientDetail">
                                 <figure>
                                    <img src="assets/img/clientIcon.png" alt="Client Image" class="img-fluid">
                                 </figure>
                                 <h5>John Doe<span>CEO, Example Company</span></h5>
                              </div>
                           </div>
                        </div>
                        <div class="swiper-slide">
                           <div class="clientReviewBox">
                              <h6 class="clientRating">
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <strong>5.0</strong>
                              </h6>
                              <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                              <div class="clientDetail">
                                 <figure>
                                    <img src="assets/img/clientIcon.png" alt="Client Image" class="img-fluid">
                                 </figure>
                                 <h5>John Doe<span>CEO, Example Company</span></h5>
                              </div>
                           </div>
                        </div>
                        <div class="swiper-slide">
                           <div class="clientReviewBox">
                              <h6 class="clientRating">
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <strong>5.0</strong>
                              </h6>
                              <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                              <div class="clientDetail">
                                 <figure>
                                    <img src="assets/img/clientIcon.png" alt="Client Image" class="img-fluid">
                                 </figure>
                                 <h5>John Doe<span>CEO, Example Company</span></h5>
                              </div>
                           </div>
                        </div>
                        <div class="swiper-slide">
                           <div class="clientReviewBox">
                              <h6 class="clientRating">
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <i class="las la-star"></i>
                                 <strong>5.0</strong>
                              </h6>
                              <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                              <div class="clientDetail">
                                 <figure>
                                    <img src="assets/img/clientIcon.png" alt="Client Image" class="img-fluid">
                                 </figure>
                                 <h5>John Doe<span>CEO, Example Company</span></h5>
                              </div>
                           </div>
                        </div>
                     </div>


                  </div>
               </div>
            </div>
            <div class="row">
               <div class="col-12 text-center">
                  <div class="swiper-button-prev"><i class="las la-long-arrow-alt-left"></i></div>
                  <div class="swiper-button-next"><i class="las la-long-arrow-alt-right"></i></div>
               </div>
            </div>
         </div>
      </section>

      <section class="myBlogs">
         <div class="container">
            <div class="row mb-5 align-items-center justify-content-between">
               <div class="col">
                  <h2 class="sectionTitle">
                     <small class="subTitle"><strong>- </strong>My Blog</small>
                     <span class="mainTitle">My Latest <em>News & Blogs</em></span>
                  </h2>
               </div>
               <div class="col text-end"><button class="btn-lg customBtn"><span>View All Blogs</span><i
                        class="las la-long-arrow-alt-right"></i></button></div>
            </div>
            <div class="row">
               <div class="col-sm-4">
                  <div class="blogCard">
                     <figure>
                        <img src="assets/img/blogImgs/thiiings.avif" alt="Project Image" class="img-fluid">
                        <figcaption><i class="las la-link"></i></figcaption>
                     </figure>
                     <h5 class="blogTags">
                        <span><i class="las la-user"></i>Carrie Cousins</span>
                        <small><i class="las la-calendar"></i>July 14, 2025</small>
                     </h5>
                     <div class="blogBody">
                        <h3 class="title-limit">Exciting New Tools for Designers.</h3>
                        <p class="text-limit-3">Summer is here, and designers are buzzing about the upcoming Apple
                           release, design changes, and trends that are coming with it. The biggest being a glass
                           effect. You’ll see some new tools for designers in this roundup that reflect that as well as
                           some new AI tools and a few fun design kits.</p>
                     </div>
                     <a href="#" class="btn-link">Read More <i class="las la-arrow-right"></i></a>
                  </div>
               </div>
               <div class="col-sm-4">
                  <div class="blogCard">
                     <figure>
                        <img src="assets/img/blogImgs/wordpress.avif" alt="Project Image" class="img-fluid">
                        <figcaption><i class="las la-link"></i></figcaption>
                     </figure>
                     <h5 class="blogTags">
                        <span><i class="las la-user"></i>Noah Davis</span>
                        <small><i class="las la-calendar"></i>July 7, 2025</small>
                     </h5>
                     <div class="blogBody">
                        <h3 class="title-limit">The Slow Implosion of WordPress.</h3>
                        <p class="text-limit-3">WordPress in 2025 isn’t thriving — it’s rotting. Bloated, outdated, and
                           hijacked by commercial greed, the world’s most popular CMS has become a cautionary tale of
                           innovation gone stale. If you’re still building on WordPress, you’re clinging to a corpse.
                        </p>
                     </div>
                     <a href="#" class="btn-link">Read More <i class="las la-arrow-right"></i></a>
                  </div>
               </div>
               <div class="col-sm-4">
                  <div class="blogCard">
                     <figure>
                        <img src="assets/img/blogImgs/design-ftr.avif" alt="Project Image" class="img-fluid">
                        <figcaption><i class="las la-link"></i></figcaption>
                     </figure>
                     <h5 class="blogTags">
                        <span><i class="las la-user"></i>Louise North</span>
                        <small><i class="las la-calendar"></i>May 27, 2025</small>
                     </h5>
                     <div class="blogBody">
                        <h3 class="title-limit">AI Agents for Designers, Developers, and UX Strategists: What You Need
                           to Know Now</h3>
                        <p class="text-limit-3">AI agents aren’t coming—they’re already quietly transforming how
                           designers, developers, and UX pros work every day. From building layouts to debugging code
                           and synthesizing user feedback, they’re the digital sidekicks doing the busywork so you can
                           focus on the big ideas.</p>
                     </div>
                     <a href="#" class="btn-link">Read More <i class="las la-arrow-right"></i></a>
                  </div>
               </div>
            </div>
         </div>
      </section>

      <section class="myFaqs">
         <div class="container">
            <div class="row mb-5">
               <div class="col-12 text-center">
                  <h2 class="sectionTitle">
                     <small class="subTitle"><strong>- </strong>FAQs</small>
                     <span class="mainTitle">Questions? <em>Look Here.</em></span>
                  </h2>
               </div>
            </div>
            <div class="row">
               <div class="col-12">
                  <div class="accordion accordion-flush faqAccordion" id="accordionFaqs">
                     <div class="accordion-item">
                        <h2 class="accordion-header">
                           <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                              data-bs-target="#faqOne" aria-expanded="true" aria-controls="faqOne">
                              Accordion Item #1
                           </button>
                        </h2>
                        <div id="faqOne" class="accordion-collapse collapse"
                           data-bs-parent="#accordionFaqs">
                           <div class="accordion-body">
                              <strong>This is the first item’s accordion body.</strong> It is shown by default, until
                              the collapse plugin adds the appropriate classes that we use to style each element. These
                              classes control the overall appearance, as well as the showing and hiding via CSS
                              transitions. You can modify any of this with custom CSS or overriding our default
                              variables. It’s also worth noting that just about any HTML can go within the though the transition does limit overflow.
                           </div>
                        </div>
                     </div>
                     <div class="accordion-item">
                        <h2 class="accordion-header">
                           <button class="accordion-button" type="button" data-bs-toggle="collapse"
                              data-bs-target="#faqTwo" aria-expanded="false" aria-controls="faqTwo">
                              Accordion Item #2
                           </button>
                        </h2>
                        <div id="faqTwo" class="accordion-collapse collapse show" data-bs-parent="#accordionFaqs">
                           <div class="accordion-body">
                              <strong>This is the second item’s accordion body.</strong> It is hidden by default, until
                              the collapse plugin adds the appropriate classes that we use to style each element. These
                              classes control the overall appearance, as well as the showing and hiding via CSS
                              transitions. You can modify any of this with custom CSS or overriding our default
                              variables. It’s also worth noting that just about any HTML can go within the
                              <code>.accordion-body</code>, though the transition does limit overflow.
                           </div>
                        </div>
                     </div>
                     <div class="accordion-item">
                        <h2 class="accordion-header">
                           <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                              data-bs-target="#faqThree" aria-expanded="false" aria-controls="faqThree">
                              Accordion Item #3
                           </button>
                        </h2>
                        <div id="faqThree" class="accordion-collapse collapse" data-bs-parent="#accordionFaqs">
                           <div class="accordion-body">
                              <strong>This is the third item’s accordion body.</strong> It is hidden by default, until
                              the collapse plugin adds the appropriate classes that we use to style each element. These
                              classes control the overall appearance, as well as the showing and hiding via CSS
                              transitions. You can modify any of this with custom CSS or overriding our default
                              variables. It’s also worth noting that just about any HTML can go within the
                              <code>.accordion-body</code>, though the transition does limit overflow.
                           </div>
                        </div>
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </section>
      <section class="linkRoller">
         <!-- Slider main container -->
         <div class="swiper">
            <!-- Additional required wrapper -->
            <div class="swiper-wrapper">
               <!-- Slides -->
               <div class="swiper-slide"><i class="las la-snowflake"></i> Graphics Design</div>
               <div class="swiper-slide"><i class="las la-snowflake"></i> Website Design</div>
               <div class="swiper-slide"><i class="las la-snowflake"></i> App Design</div>
               <div class="swiper-slide"><i class="las la-snowflake"></i> Product Design</div>
               <div class="swiper-slide"><i class="las la-snowflake"></i> UI/UX Design</div>
               <div class="swiper-slide"><i class="las la-snowflake"></i> Responsive Design</div>
            </div>

         </div>
      </section>
      <!--Footer Start Here-->
      <?php include( "includes/footer.php" ); ?>
      <!--Footer Ends Here-->

   </main>
</body>

</html>